...
class Solution {
    ...
    private Pair<Integer, Integer> slope_coprime(int x1, int y1, int x2, int y2) {
        ...
        else if (deltaY == 0) return new Pair<>(Integer.MAX_VALUE, Integer.MAX_VALUE);
        else if (deltaX < 0) {
            deltaX = -deltaX;
            deltaY = -deltaY;
        }
    ...
    public int max_points_on_a_line_pti (int i) {
        lines.clear();
        horizontal_lines = 1;
        int count = 1;
        int duplicates = 0;
    ...
    public int maxPoints(int[][] points) {
        ...
        int maxP = 1;
        for (int i = 0; i < n - 1; i++)
            maxP = Math.max(max_points_on_a_line_pti(i), maxP);
        return maxP;
    }
